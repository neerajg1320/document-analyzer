...
def upload(file)
  ...
  if (file[:password])
    Tabula::Workspace.instance.save_document_password(file_id, file[:password])

    Tabula::Workspace.instance.get_document_password(file_id)
  end
  ...
  on post do
    on 'upload.json' do
      ...
      if req.params['file'] # single upload mode. this should be deleting once if decide to enable multiple upload for realzies
        #NG Start
        puts req.params['file'][:filename]
        puts req.params['file'][:filepassword]
        #NG End
        ...
        statuses = req.params['files'].map do |file|
          #NG 2018-07-06 4:15pm Added the password for a file
          file[:password] = req.params['filepassword']
    ...
    on "pdf/:file_id/data" do |file_id|
      ...
      #NG: Get the stored password
      pdf_password = Tabula::Workspace.instance.get_document_password(file_id)
      ...
      #NG debug
      puts coords
      puts "pdf/:file_id/data :  " + pdf_password

      tables = Tabula.extract_tables(pdf_path, pdf_password, coords)
      ...
      when 'json'
        ...
        tables.each_with_index do |table, index|
          res.write ", " if index > 0
          strbuf = table.to_json[0...-1] + ", \"spec_index\": #{table.spec_index}}"
          #NG debug
