...
module Tabula
  class Workspace
    ...
    def move_file(path, document_id, filename)
      dst_file = File.join(get_document_dir(document_id), filename)
      FileUtils.mv(path, dst_file)
      puts "File Created: " +  dst_file
    end

    #NG Start 2018-07-06
    def save_document_password(document_id, password)
      p = get_document_dir(document_id)
      filename = "password.txt"
      passwd_file = File.join(p, filename)

      File.open(passwd_file, 'w') do |f|
        f.write password
      end

      #puts "Password File Created: " +  passwd_file
    end

    def get_document_password(document_id)
      p = get_document_dir(document_id)
      filename = "password.txt"
      passwd_file = File.join(p, filename)
      ...
      #puts "Password File Read: " +  passwd_file + ": " + password
      return password
    end
    #NG End
