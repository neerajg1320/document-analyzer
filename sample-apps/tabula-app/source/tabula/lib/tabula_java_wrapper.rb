...
module Tabula
  ...
  def Tabula.extract_tables(pdf_path, pdf_password, specs, options={})
    options = {
    ...
    extractor = Extraction::ObjectExtractor.new(pdf_path, :all, password=pdf_password)
  ...
  module Extraction
    ...
    def Extraction.openPDF(pdf_filename, password='')
      ...
      #NG Fix TBR and made proper
      puts "Extraction.openPDF: " + password
      #PDDocument.load(java.io.File.new(pdf_filename), "NEE0306")
      PDDocument.load(java.io.File.new(pdf_filename), password)
    end
    ...
    class ObjectExtractor < Java::TechnologyTabula.ObjectExtractor
      ...
      def initialize(pdf_filename, pages=[1], password='', options={})
        ...
        #NG debug
        puts "ObjectExtractor.initialize: " + password

        raise Errno::ENOENT unless File.exists?(pdf_filename)
        @pdf_filename = pdf_filename
        @document = Extraction.openPDF(pdf_filename, password=password)
